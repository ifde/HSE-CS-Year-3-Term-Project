#let template(cfg: none, body) = {
  ...
  let outline_and_normal_pages = {
    ...
    // <-- INDENTATION RULES START HERE (added for levels 3, 4, 5)
    show heading.where(level: 3): h => {
      set text(weight: "bold", size: 12pt)

      pad(left: 10pt)[#counter(heading).display() #h.body]
    }

    show heading.where(level: 4): h => {
      set text(weight: "bold", size: 12pt)

      pad(left: 20pt)[#counter(heading).display() #h.body]
    }

    show heading.where(level: 5): h => {
      set text(weight: "bold", size: 12pt)

      pad(left: 30pt)[#counter(heading).display() #h.body]
    }
    // <-- INDENTATION RULES END HERE
